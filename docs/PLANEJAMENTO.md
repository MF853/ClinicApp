...
Atualizado em 10/09/2026. Projeto pessoal, sem vínculo com outros projetos empresariais. Este arquivo registra o plano e a situação da entrega; não substitui nem altera o [documento de requisitos](Documento-de-Requisitos-ClinicApp-v2.md).
...
| 2. Persistência e identidade | Modelo, migrations, autenticação, vínculos, clínica/perfil ativo | Base implementada. PostgreSQL real, sessões persistidas, Argon2id, CSRF, cadastro administrativo via API, recuperação de senha e alternância. Ainda falta completar cadastros/vínculos e testes de todos os endpoints. |
...
| 5. Interfaces, comunicação e mobile | Telas por perfil, SSE, notificações e Capacitor | Web React conectada; SSE e e-mail local implementados. Capacitor apenas dependências/configuração, sem projeto nativo compilado. Comunicação direta, push e SMS pendentes. |
...
- Consultar pacientes, registrar decisão humana de aplicar/suspender consequência e alterar parâmetros globais básicos.
...
- Cadastro público de terapeuta, convite/autocadastro de paciente e telas completas de cadastro, vinculação e desvinculação (RF-ADM-01/02/07).
...
- Edição integral da grade, férias/feriados/bloqueios recorrentes, alterações solicitadas pela clínica, exceções manuais completas e liberação de pendência sem penalidade (RF-TER-01/04/06, RN-15).
- Parâmetros individuais e configuração completa de calendário, janela, retenção, reposições e alertas clínicos, com telas e validações. Reativação precisa reavaliar vagas explicitamente.
...
- Mensagens diretas, alertas gerais na interface, preferências/janela de silêncio, lembretes e provedores reais de push/SMS/e-mail. E-mail local capturado não comprova entrega externa.
...
- Baselines de telas/componentes críticos e matriz por risco em 320/390/768/1440. Executar Firefox/WebKit quando os motores estiverem instalados, além de testes Safari/iOS reais.
