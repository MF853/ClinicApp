...
export class ApiController {
  ...
  @Post('parameters') parameters(@Req() r: AuthRequest, @Body() b: ParametersDto) { roles(r.context, 'ADMIN'); return transaction(r.context, async tx => { const result = await tx.clinic.update({ where: { id: r.context.clinicId }, data: b }); await audit(tx, r.context, 'parameters-updated', r.context.clinicId, { before: Object.fromEntries(Object.keys(b).map(key => [key, r.context.clinic[key as keyof typeof r.context.clinic]])), after: { ...b } }); return result; }); }
