...
export class ApiController {
  ...
  @Get('fittings') async fittings(@Req() r: AuthRequest) { const rows = await db.fittingRequest.findMany({ where: { clinicId: r.context.clinicId, ...(r.context.role === 'PATIENT' ? { patientId: r.context.id } : {}) }, orderBy: { createdAt: 'desc' } }); const result = []; for (const row of rows) { try { await appointment(db, r.context, row.originalId); result.push(row); } catch (e) { if (!(e instanceof ForbiddenException)) throw e; } } return result; }
