import { IsString, IsIn, IsUUID, IsOptional, MaxLength, MinLength, IsInt, Min, Max, IsBoolean, IsEmail, IsDateString, Matches, ValidateIf, IsArray, ArrayUnique, ArrayMaxSize } from 'class-validator';
import { categories } from '../absences/certificates.js';
...
export class ParametersDto { @ApiProperty() @IsInt() @Min(1) @Max(100) absenceLimit!: number; @ApiProperty() @IsInt() @Min(1) @Max(30) justificationDays!: number; @ApiProperty() @IsInt() @Min(0) @Max(23) confirmationHour!: number; @ApiProperty() @IsInt() @Min(2) @Max(24) closeHours!: number;
  @ApiPropertyOptional() @ValidateIf((_o, v) => v !== undefined) @IsInt() @Min(1) @Max(3650) evaluationDays?: number;
  @ApiPropertyOptional() @ValidateIf((_o, v) => v !== undefined) @IsInt() @Min(1) @Max(100) replacementPerAbsence?: number;
  @ApiPropertyOptional() @ValidateIf((_o, v) => v !== undefined) @IsInt() @Min(1) @Max(1000) replacementPerWindow?: number;
  @ApiPropertyOptional() @ValidateIf((_o, v) => v !== undefined) @IsArray() @ArrayUnique() @ArrayMaxSize(1000) @IsDateString({ strict: true }, { each: true }) @Matches(/^\d{4}-\d{2}-\d{2}$/, { each: true }) holidays?: string[];
