...
export async function systemTransaction<T>(clinicId: string, fn: (tx: Tx, ctx: Context) => Promise<T>): Promise<T> {
  return db.$transaction(async tx => {
    ...
    const ctx: Context = { id: 'SYSTEM', userId: 'SYSTEM', clinicId, role: 'ADMIN', active: true, canReview: true, birthDate: null, registration: '', absenceLimit: null, justificationDays: null, resetAt: null, suspendedUntil: null, clinic, user: { id: 'SYSTEM', name: 'Sistema', email: '' } };
    return fn(tx, ctx);
  }, { timeout: 20000 });
}
